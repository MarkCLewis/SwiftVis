...
public class CartAndRadSource extends AbstractSource implements FileSourceInter {
    ...
    public int getNumValues(int stream) {
        return includesSpin ? 10 : 7;
    }

    @Override
    public String getValueDescription(int stream, int which) {
        String[] vdesc={"x","y","z","vx","vy","vz","radius","wx","wy","wz"};
        return vdesc[which];
    }
    ...
    private void redoBuffer(int index) {
        try {
            if(raf==null) {
                ...
                includesSpin=(totalNum*8l*(6l+1l+3l)+4l == raf.length());
            }
            dataVect.clear();
            long maxBufferSize=OptionsData.instance().getSourceSize()/(7*4+30);
            ArrayList<ArrayIndexPair> pairs=new ArrayList<ArrayIndexPair>();
            int[] params=new int[0];
            float[] vals=new float[includesSpin ? 10 : 7];
            ...
            for(int i=index; i<totalNum && (pairs.size()<maxBufferSize || scanningAll); ++i) {
                ...
                if((!useThin.getValue() || i%oneIn.getValue()==0) && 
                    ...
                    if(pairs.size()<maxBufferSize) {
                        ...
                        vals=new float[includesSpin ? 10 : 7];
                    }
                    totalElements++;
                }
            }
            int pairIndex=0;
            raf.seek(4l+totalNum*6l*8l+index*8l);
            for(int i=index; i<totalNum && pairIndex<pairs.size(); ++i) {
                ...
                if(pairs.get(pairIndex).index==i) {
                    ...
                    if(!includesSpin) dataVect.add(new DataElement(params,pairs.get(pairIndex).value));
                    pairIndex++;
                }
            }
            if(includesSpin) {
                pairIndex=0;
                raf.seek(4l+totalNum*6l*8l+totalNum*8l+index*3l*8l);
                for(int i=index; i<totalNum && pairIndex<pairs.size(); ++i) {
                    float sx=(float)bi.readReal8();
                    float sy=(float)bi.readReal8();
                    float sz=(float)bi.readReal8();
                    if(pairs.get(pairIndex).index==i) {
                        pairs.get(pairIndex).value[7]=sx;
                        pairs.get(pairIndex).value[8]=sy;
                        pairs.get(pairIndex).value[9]=sz;
    ...
    private transient boolean includesSpin;
